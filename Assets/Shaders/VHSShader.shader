Shader "Hidden/VHSShader"
{
    Properties
    {
        ...
        _NoiseTextureTime("Noise Texture Time", float) = 1
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            fixed4 frag(v2f i) : SV_Target
            {
                ...
                float noise = tex2D(_NoiseTexture, uv * _NoiseTexture_ST.xy + rand(_Time.x * _NoiseTextureTime)).x;
                ...
                //Passing lines noise
                col += saturate(mul(step(0.99, 1.0 - randLines) * step(sineLines, noise) * 0.2, 0.5));
                return col;
            }
            ENDCG
        }
